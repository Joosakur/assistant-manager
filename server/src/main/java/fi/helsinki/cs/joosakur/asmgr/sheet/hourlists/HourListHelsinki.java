...
public class HourListHelsinki extends HourList {

    public HourListHelsinki(Sheet sheet) {
        super(sheet);
    }
    ...
    public HourList populate(Assistant assistant, Stream<AtomicSpan> atomicSpans) {
        ...
        sheet.getCellAt(2,38).setValue(LocalDate.now().format(DateTimeFormatter.ofPattern("dd.MM.yyyy")));
        sheet.getCellAt(1, 41).setValue(assistant.getFirstName()+" "+assistant.getLastName());
        sheet.getCellAt(5, 41).setValue(employer.getFirstName()+" "+employer.getLastName());
        ...
        final int SUMMARY_ROW = 36;
        sheet.getCellAt(2, SUMMARY_ROW).setValue(workDays);
